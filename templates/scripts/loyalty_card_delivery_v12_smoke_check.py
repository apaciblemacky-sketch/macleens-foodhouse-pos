...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix='mfh-loyalty-v12-') as folder:
        ...
        with m.app.app_context():
            ...
            target = m.Customer(name='New Member', contact='09982220000', pin_hash=generate_password_hash('1234'), points_balance=0, accumulated_spend=0, card_number='MFH-0002')
            ...
            # A phone scanner only hands a customer to the same cashier's POS.
            # It cannot create a duplicate order or award points by itself.
            phone = m.app.test_client()
            laptop = m.app.test_client()
            for browser in (phone, laptop):
                with browser.session_transaction() as staff_session:
                    staff_session['cashier_user'] = 'cashier1'
                    staff_session['_staff_last_activity'] = datetime.now().isoformat()
            scanner_page = phone.get('/pos/mobile-scanner')
            assert scanner_page.status_code == 200 and b'Send member to laptop Cashier POS' in scanner_page.data
            lookup = phone.post('/api/mobile-scanner/lookup', json={'scan_value': 'https://example.test/portal/login?card=MFH-0002'})
            assert lookup.status_code == 200 and lookup.get_json()['customer']['id'] == target.id
            queued = phone.post('/api/mobile-scanner/queue', json={'scan_value': 'MFH-0002'})
            assert queued.status_code == 200 and queued.get_json()['success'] is True
            pending_scan = laptop.get('/api/mobile-scanner/pending')
            pending_data = pending_scan.get_json()
            assert pending_scan.status_code == 200 and pending_data['scan']['customer_id'] == target.id
            claim = laptop.post(f"/api/mobile-scanner/{pending_data['scan']['id']}/claim")
            assert claim.status_code == 200 and claim.get_json()['success'] is True
            assert laptop.get('/api/mobile-scanner/pending').get_json()['scan'] is None
            assert m.Order.query.count() == 2
            assert m.db.session.get(m.Customer, target.id).points_balance == 25
    ...
    print('LOYALTY ₱40, MOBILE QR SCANNER, DELIVERY 30/65, 2X2 CARD, REDEMPTION, AND BIR SALES RECORD SMOKE CHECK PASSED')
    return 0
