...
REQUIRED_DB_COLUMNS = {
    ...
    "community_profile": {
        ...
        "student_application_graduating_year", "public_bio", "is_profile_locked", "cover_image_data",
...
def main() -> int:
    ...
    digital_markers = [
        ...
        "digital_gcash_gateway_mode",
        "digital_paypal_checkout_enabled",
        "paymongo_active",
        "paypal_available",
    ...
    for marker in ["loyalty_spend_per_point", "daily_login_points", "customer_has_early_redemption_qualifying_purchase", "ensure_loyalty_and_delivery_upgrade_defaults", "@app.route('/admin/customer/<int:cust_id>/purchase-history')", "@app.route('/admin/customer/<int:cust_id>/reinstate-old-order'", "@app.route('/admin/bir-sales-record')", "receipt_number", "digital_order_download_asset", "asset_version", "class MobileLoyaltyScan", "@app.route('/pos/mobile-scanner')", "@app.route('/api/mobile-scanner/queue'"]:
        if marker not in source:
            fail(f"v12 loyalty/history server marker is missing: {marker}")
    mobile_scanner = TEMPLATES / "mobile_scanner.html"
    if not mobile_scanner.exists():
        fail("mobile loyalty scanner template is missing")
    else:
        scanner_text = mobile_scanner.read_text(encoding="utf-8")
        for marker in ["Start camera scanner", "Send member to laptop Cashier POS", "BarcodeDetector", "/api/mobile-scanner/lookup", "/api/mobile-scanner/queue"]:
            if marker not in scanner_text:
                fail(f"mobile loyalty scanner UI marker is missing: {marker}")
    ...
    for marker in [
        ...
        "buildCompactBulkCatalogPayload", "body: compactSubmission.payload", "Compact Save v5",
    ]:
    ...
    for marker in [
        ...
        "MAX_FORM_PARTS", "RequestEntityTooLarge", "APP_RELEASE",
    ]:
        if marker not in source:
    ...
    for marker in [
        ...
        "suggestedPeople", "Lock my profile", "Project workspaces", "No points for likes or follows",
        "coverPhotoInput", "Tag up to", "communityGroupCreateForm", "respondGroupInvite", "data-community-tab=\"projects\"",
    ...
    for marker in ["Community Lite safety controls", "Priority verification and safety alerts", "Moderation review", "8:00 AM Flash Poll", "Flash Perch alerts", "Native in-feed ads", "Safety phrase holds"]:
    ...
    for marker in [
        "Project monitoring workspace", "Project progress", "progressPercent", "taskForm", "noteForm", "pollForm",
        "assigned_to_profile_id", "Facebook Messenger link",
        "Leave this project", "Results update immediately without refreshing",
    ]:
        if marker not in group_text:
            fail(f"Community group-workspace marker is missing: {marker}")
    for retired_chat_marker in ['id="messageForm"', 'data-panel="chat"', "pollMessages", "/messages"]:
        if retired_chat_marker in group_text:
            fail(f"Retired internal-chat UI marker is still present: {retired_chat_marker}")
