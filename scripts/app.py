...
class MobileLoyaltyScan(db.Model):
    ...
    The scan selects a member only. It deliberately does not create an order
    or award points: the existing Cashier POS sale remains the single source
    of truth for receipts, sales totals, stock, and loyalty earning.
    """
    __tablename__ = 'mobile_loyalty_scan'
    id = db.Column(db.Integer, primary_key=True)
    customer_id = db.Column(db.Integer, db.ForeignKey('customer.id', ondelete='CASCADE'), nullable=False, index=True)
    staff_username = db.Column(db.String(50), nullable=False, index=True)
    status = db.Column(db.String(20), nullable=False, default='PENDING', index=True)
    created_at = db.Column(db.DateTime, default=utc_now, nullable=False, index=True)
    claimed_at = db.Column(db.DateTime, nullable=True)
    customer = db.relationship('Customer', backref=db.backref('mobile_loyalty_scans', lazy=True))
...
# A phone-to-laptop scan is intentionally short-lived.  It only selects a
# member in the Cashier POS; it never creates a second sale or changes points.
MOBILE_LOYALTY_SCAN_TTL_SECONDS = 180


def active_cashier_username():
    """Return the current staff identity shared by phone and Cashier POS."""
    return (session.get('cashier_user') or session.get('admin_user') or '').strip()


def loyalty_card_code_from_scan(raw_value):
    """Extract a loyalty card identifier from raw QR text or a card-login URL."""
    value = unquote_plus(str(raw_value or '').strip())
    if not value:
        return ''
    try:
        parsed = urlparse(value)
        parameters = parse_qs(parsed.query or '')
        for key in ('card', 'card_number', 'member', 'member_card'):
            selected = parameters.get(key, [''])[-1]
            if selected:
                value = unquote_plus(str(selected).strip())
                break
    except (TypeError, ValueError):
        pass
    return re.sub(r'\s+', '', value)[:80].upper()


def customer_from_loyalty_card_scan(raw_value):
    """Find a customer by loyalty card only; a scanned card must not expose phone lookup."""
    card_code = loyalty_card_code_from_scan(raw_value)
    if not card_code:
        return None, ''
    customer = Customer.query.filter(db.func.upper(Customer.card_number) == card_code).first()
    return customer, card_code


def expire_mobile_loyalty_scans(staff_username):
    """Expire unclaimed selections so a later customer cannot be loaded by mistake."""
    cutoff = utc_now() - timedelta(seconds=MOBILE_LOYALTY_SCAN_TTL_SECONDS)
    return MobileLoyaltyScan.query.filter(
        MobileLoyaltyScan.staff_username == staff_username,
        MobileLoyaltyScan.status == 'PENDING',
        MobileLoyaltyScan.created_at < cutoff,
...
def mobile_loyalty_scan_payload(scan):
    """Return only the staff-safe details needed to select a member at POS."""
    customer = scan.customer or db.session.get(Customer, scan.customer_id)
    if not customer:
        return None
    return {
        'id': scan.id,
        ...
        'points_balance': round(parse_float(customer.points_balance, 0.0), 2),
        'created_at': scan.created_at.isoformat() if scan.created_at else None,
    }


@app.route('/pos/mobile-scanner')
@require_cashier
def cashier_mobile_scanner():
    """Phone-first loyalty card scanner for the currently logged-in cashier."""
    return render_template(
        'mobile_scanner.html',
        staff_username=active_cashier_username(),
        scan_ttl_seconds=MOBILE_LOYALTY_SCAN_TTL_SECONDS,
    )


@app.route('/api/mobile-scanner/lookup', methods=['POST'])
@require_cashier
def mobile_scanner_lookup():
    data = request.get_json(silent=True) or {}
    customer, card_code = customer_from_loyalty_card_scan(data.get('scan_value'))
    if not card_code:
        return jsonify({'success': False, 'message': 'Scan a loyalty QR or enter the MFH card number.'}), 400
    if not customer:
        return jsonify({'success': False, 'message': 'No active member was found for that loyalty card.'}), 404
    ...
    return jsonify({
        ...
        'customer': {
            'id': customer.id,
            'name': customer.name,
            'card_number': customer.card_number or card_code,
            'points_balance': round(parse_float(customer.points_balance, 0.0), 2),
        },
    })


@app.route('/api/mobile-scanner/queue', methods=['POST'])
@require_cashier
def mobile_scanner_queue():
    """Send the scanned member to the same cashier user's open POS screen."""
    data = request.get_json(silent=True) or {}
    customer, card_code = customer_from_loyalty_card_scan(data.get('scan_value'))
    if not card_code:
        return jsonify({'success': False, 'message': 'Scan a loyalty QR or enter the MFH card number.'}), 400
    ...
    if not staff_username:
        return jsonify({'success': False, 'message': 'Staff sign-in is required to send a scan.'}), 401
    now = utc_now()
    expire_mobile_loyalty_scans(staff_username)
    # Only the latest scan may be loaded. This prevents an old card from being
    # accidentally attached to a later customer order.
    MobileLoyaltyScan.query.filter_by(staff_username=staff_username, status='PENDING').update(
        {'status': 'SUPERSEDED', 'claimed_at': now}, synchronize_session=False
    )
    scan = MobileLoyaltyScan(customer_id=customer.id, staff_username=staff_username, status='PENDING')
    db.session.add(scan)
    db.session.add(PortalEvent(source='MOBILE_SCANNER', event_type='LOYALTY_CARD_QUEUED', customer_id=customer.id))
    db.session.commit()
    return jsonify({
        'success': True,
        'message': 'Member sent to the laptop Cashier POS.',
        'scan': mobile_loyalty_scan_payload(scan),
        'expires_in_seconds': MOBILE_LOYALTY_SCAN_TTL_SECONDS,
    })


@app.route('/api/mobile-scanner/pending')
@require_cashier
def mobile_scanner_pending():
    """Let the laptop POS poll only scans made by the same staff account."""
    staff_username = active_cashier_username()
    expired = expire_mobile_loyalty_scans(staff_username)
    scan = MobileLoyaltyScan.query.filter_by(
        staff_username=staff_username,
        status='PENDING',
    ).order_by(MobileLoyaltyScan.created_at.desc(), MobileLoyaltyScan.id.desc()).first()
    if expired:
        db.session.commit()
    response = jsonify({'success': True, 'scan': mobile_loyalty_scan_payload(scan) if scan else None})
    response.headers['Cache-Control'] = 'no-store, max-age=0'
    return response


@app.route('/api/mobile-scanner/<int:scan_id>/claim', methods=['POST'])
@require_cashier
def mobile_scanner_claim(scan_id):
    """Mark a handoff consumed after the laptop has selected the member."""
    staff_username = active_cashier_username()
    expire_mobile_loyalty_scans(staff_username)
    scan = MobileLoyaltyScan.query.filter_by(
        id=scan_id,
    ...
    if not scan:
        db.session.commit()
        return jsonify({'success': False, 'message': 'This phone scan is no longer available.'}), 404
    scan.status = 'CLAIMED'
    scan.claimed_at = utc_now()
    db.session.add(PortalEvent(source='MOBILE_SCANNER', event_type='LOYALTY_CARD_LOADED', customer_id=scan.customer_id))
    db.session.commit()
