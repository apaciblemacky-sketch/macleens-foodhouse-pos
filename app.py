...
class Product(db.Model):
    ...
    comments = db.relationship('ProductComment', backref='product_rel', cascade="all, delete-orphan", lazy=True, order_by="desc(ProductComment.created_at)")[cite: 12]
...
def make_session_permanent():
    session.permanent = True[cite: 12]
...
def api_queue_counts():
    try:
        pending_cashier = Order.query.filter_by(status="VERIFICATION").count()[cite: 12]
    except Exception:
        pending_cashier = 0
    return jsonify({'pending_cashier': pending_cashier})[cite: 12]
...
def store_catalog():
    ...
    top_customers = Customer.query.order_by(Customer.points_balance.desc()).limit(10).all()[cite: 12]
    ...
    return render_template('store_catalog.html', 
                           ...
                           total_accumulated_visits=total_accumulated_visits)[cite: 4, 12]
...
def cashier_terminal():
    ...
    pending_orders = Order.query.filter_by(status="VERIFICATION").order_by(Order.created_at.asc()).all()[cite: 12]
    completed_orders = Order.query.filter_by(status="COMPLETED").order_by(Order.created_at.desc()).limit(15).all()[cite: 12]
    unpaid_collections = Order.query.filter_by(is_unpaid=True).order_by(Order.created_at.desc()).all()[cite: 12]
    ...
    try:
        online_customers = Customer.query.filter(Customer.last_active_at >= two_mins_ago).order_by(Customer.last_active_at.desc()).all()[cite: 12]
    except Exception:
        online_customers = [][cite: 12]

    credit_customers = Customer.query.filter(Customer.outstanding_ar > 0).order_by(Customer.outstanding_ar.desc()).all()[cite: 12]

    today = date.today()[cite: 12]
    today_expenses = Expense.query.filter(db.func.date(Expense.created_at) == today).order_by(Expense.created_at.desc()).all()[cite: 12]
    today_drops = VaultDrop.query.filter(db.func.date(VaultDrop.created_at) == today).order_by(VaultDrop.drop_number.asc()).all()[cite: 12]
    try:
        today_change_funds = ChangeFund.query.filter(db.func.date(ChangeFund.created_at) == today).order_by(ChangeFund.created_at.desc()).all()[cite: 12]
    except Exception:
        today_change_funds = [][cite: 12]
    ...
    return render_template('cashier_pos.html', 
                           ...
                           next_drop_num=next_drop_num)[cite: 11, 12]
...
def admin_revert_order(order_id):
    ...
    db.session.commit()[cite: 12]
    flash(f"Order #{order_id} deleted & reverted.", "info")[cite: 12]
    return redirect(url_for('admin_dashboard'))[cite: 12]
...
def admin_purge_dummy_orders(cust_id):
    ...
    db.session.commit()[cite: 12]
    flash(f"Purged test orders & reset points for '{cust.name}'.", "info")[cite: 12]
    return redirect(url_for('admin_dashboard'))[cite: 12]
...
def admin_add_product():
    ...
    if not name or price <= 0:
        flash("Please enter a valid product name and price.", "error")[cite: 12]
        return redirect(url_for('admin_dashboard'))[cite: 12]
    ...
    db.session.commit()[cite: 12]
    flash(f"Product '{name}' added successfully!", "success")[cite: 12]
    return redirect(url_for('admin_dashboard'))[cite: 12]
...
def admin_toggle_promo(promo_id):
    ...
    db.session.commit()[cite: 12]
    flash(f"Campaign '{promo.title}' status updated.", "success")[cite: 12]
    return redirect(url_for('admin_dashboard'))[cite: 12]
...
def admin_toggle_promo_visibility(promo_id):
    ...
    db.session.commit()[cite: 12]
    flash(f"Promo '{promo.title}' visibility updated.", "info")[cite: 12]
    return redirect(url_for('admin_dashboard'))[cite: 12]
...
# Safe database startup execution
...
if __name__ == '__main__':
    app.run(debug=True, port=5000)[cite: 12]