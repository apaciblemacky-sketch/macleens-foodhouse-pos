...
# ==================== SAFE AUTO-MIGRATIONS & BEFORE REQUEST ====================

def run_isolated_migrations():
    global DB_MIGRATED
    if DB_MIGRATED:
        return
    ...
    try:
        ...
        if dialect == 'postgresql':
            statements = [
                ...
                'ALTER TABLE product ADD COLUMN IF NOT EXISTS cost DOUBLE PRECISION DEFAULT 0.0;',
            ...
            statements = [
                ...
                'ALTER TABLE customer ADD COLUMN last_active_at TIMESTAMP;',
                'ALTER TABLE "order" ADD COLUMN dining_option VARCHAR(20) DEFAULT \'DINE-IN\';',
                'ALTER TABLE promotion_tracker ADD COLUMN is_visible BOOLEAN DEFAULT 1;',
                'ALTER TABLE product ADD COLUMN cost FLOAT DEFAULT 0.0;',
...
@app.before_request
def make_session_permanent_and_migrate():
...
def cashier_topup_member_wifi():
    cust_id = request.form.get('customer_id')[cite: 13]
    if not cust_id:
        flash("Please select a member.", "error")[cite: 13]
        return redirect(url_for('cashier_terminal'))[cite: 13]
    
    cust = Customer.query.get_or_404(int(cust_id))[cite: 13]
    cust.wifi_minutes_left = (cust.wifi_minutes_left or 0) + 10[cite: 13]
    if not cust.wifi_voucher_code:
        cust.wifi_voucher_code = "MFH-" + ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))[cite: 13]
    
    db.session.commit()[cite: 13]
    flash(f"📶 Stacked +10 Mins Wi-Fi for {cust.name}! (Total: {cust.wifi_minutes_left} mins)", "success")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]
...
def cashier_direct_sale():
    ...
    cust_type = data.get('customer_type', 'WALKIN')[cite: 13]
    reg_id = data.get('registered_customer_id')[cite: 13]
    dining_opt = data.get('dining_option', 'DINE-IN').upper()[cite: 13]
    pay_method = data.get('payment_method', 'CASH').upper()[cite: 13]
    cust_name = data.get('customer_name', 'Counter Walk-in').strip() or 'Counter Walk-in'[cite: 13]
    notes = data.get('notes', 'Cashier Counter POS Sale').strip() or 'Cashier Counter POS Sale'[cite: 13]
    change_for = float(data.get('change_for') or 0.0)[cite: 13]

    if not items:
        return jsonify({'success': False, 'message': 'No items in cart.'}), 400[cite: 13]
    ...
    contact = 'N/A'[cite: 13]
    points_earned = 0[cite: 13]

    if cust_type == 'REGISTERED' and reg_id:
        cust = Customer.query.get(reg_id)[cite: 13]
        if cust:
            cust_id = cust.id[cite: 13]
            cust_name = cust.name[cite: 13]
            contact = cust.contact[cite: 13]
            cust.accumulated_spend = (cust.accumulated_spend or 0.0) + subtotal[cite: 13]
            points_earned = int(subtotal // 30)[cite: 13]
            if points_earned > 0:
                cust.points_balance = (cust.points_balance or 0.0) + points_earned[cite: 13]
...
def cashier_claim_promo():
    ...
    promo_code = request.form.get('promo_code', 'BURGER_FRIES_50')[cite: 13]
    reg_id = request.form.get('registered_customer_id')[cite: 13]
    dining_opt = request.form.get('dining_option', 'DINE-IN').upper()[cite: 13]
    pay_method = request.form.get('payment_method', 'CASH').upper()[cite: 13]
    
    promo = PromotionTracker.query.filter_by(promo_code=promo_code).first()[cite: 13]
    if not promo:
        flash("Promotion deal not found.", "error")[cite: 13]
        return redirect(url_for('cashier_terminal'))[cite: 13]

    cust_id = None[cite: 13]
    cust_name = 'Walk-in Member'[cite: 13]
    contact = 'N/A'[cite: 13]
    points_earned = 0[cite: 13]
    ...
    if reg_id:
        ...
        if cust:
            ...
            cust_name = cust.name[cite: 13]
            contact = cust.contact[cite: 13]
            cust.accumulated_spend = (cust.accumulated_spend or 0.0) + promo.promo_price[cite: 13]
            points_earned = int(promo.promo_price // 30)[cite: 13]
            if points_earned > 0:
                cust.points_balance = (cust.points_balance or 0.0) + points_earned[cite: 13]
                db.session.add(RewardLedger(
                    customer_id=cust.id,
    ...
    promo.claims_count = (promo.claims_count or 0) + 1[cite: 13]
    promo.total_revenue = (promo.total_revenue or 0.0) + promo.promo_price[cite: 13]

    db.session.commit()[cite: 13]
    flash(f"🎉 Promo Deal '{promo.title}' recorded ({dining_opt}) for {cust_name} (₱{promo.promo_price:,.2f})!", "success")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]

@app.route('/pos/create-reservation', methods=['POST'])
@require_cashier
def cashier_create_reservation():
    cust_type = request.form.get('customer_type', 'REGISTERED')[cite: 13]
    reg_id = request.form.get('registered_customer_id')[cite: 13]
    product_id = request.form.get('product_id')[cite: 13]
    qty = int(request.form.get('quantity') or 1)[cite: 13]
    dining_opt = request.form.get('dining_option', 'TAKEOUT').upper()[cite: 13]
    target_time = request.form.get('target_time', '').strip() or 'Today'[cite: 13]
    pay_method = request.form.get('payment_method', 'CASH').upper()[cite: 13]
    notes = request.form.get('notes', '').strip() or 'In-Store Reservation'[cite: 13]

    if not product_id or qty <= 0:
        flash("Please select a valid product and quantity.", "error")[cite: 13]
        return redirect(url_for('cashier_terminal'))[cite: 13]

    prod = Product.query.get_or_404(int(product_id))[cite: 13]
    subtotal = prod.price * qty[cite: 13]

    cust_id = None[cite: 13]
    cust_name = 'Walk-in Guest'[cite: 13]
    contact = 'N/A'[cite: 13]

    if cust_type == 'REGISTERED' and reg_id:
        cust = Customer.query.get(int(reg_id))[cite: 13]
        if cust:
            cust_id = cust.id[cite: 13]
            cust_name = cust.name[cite: 13]
            contact = cust.contact[cite: 13]
    else:
        cust_name = request.form.get('custom_customer_name', '').strip() or 'Walk-in Reservation'[cite: 13]
        contact = request.form.get('custom_contact', '').strip() or 'N/A'[cite: 13]
    ...
    if prod.stock >= qty:
        prod.stock -= qty[cite: 13]

    db.session.commit()[cite: 13]
    flash(f"📌 Reserved {prod.name} x{qty} ({dining_opt}) for {cust_name} (Pickup: {target_time}) — ₱{subtotal:,.2f}", "success")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]

@app.route('/pos/misc-sale', methods=['POST'])
@require_cashier
def cashier_misc_sale():
    service_name = request.form.get('service_name', '').strip() or 'Printing / Custom Service'[cite: 13]
    amount = float(request.form.get('amount') or 0.0)[cite: 13]
    pay_method = request.form.get('payment_method', 'CASH')[cite: 13]
    notes = request.form.get('notes', '').strip() or 'Over-the-counter Misc Service'[cite: 13]
    reg_cust_id = request.form.get('registered_customer_id')[cite: 13]
    custom_name = request.form.get('custom_customer_name', '').strip()[cite: 13]

    if amount <= 0:
        flash("Amount must be greater than zero.", "error")[cite: 13]
        return redirect(url_for('cashier_terminal'))[cite: 13]

    cust_id = None[cite: 13]
    customer_name = 'Walk-in Customer'[cite: 13]
    contact = 'N/A'[cite: 13]

    if reg_cust_id:
        cust = Customer.query.get(reg_cust_id)[cite: 13]
        if cust:
            cust_id = cust.id[cite: 13]
            customer_name = cust.name[cite: 13]
            contact = cust.contact[cite: 13]
            cust.accumulated_spend = (cust.accumulated_spend or 0.0) + amount[cite: 13]
            earned = int(amount // 30)[cite: 13]
            if earned > 0:
                cust.points_balance = (cust.points_balance or 0.0) + earned[cite: 13]
                db.session.add(RewardLedger(customer_id=cust.id, points_change=earned, reason=f"Service Purchase: {service_name}"))[cite: 13]
    elif custom_name:
        customer_name = custom_name[cite: 13]
    ...
    db.session.commit()[cite: 13]
    flash(f"Misc Sale recorded: {service_name} for {customer_name} (₱{amount:,.2f})", "success")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]

@app.route('/pos/record-change-fund', methods=['POST'])
@require_cashier
def cashier_record_change_fund():
    title = request.form.get('fund_title', 'Opening Petty/Change Fund').strip()[cite: 13]
    amount = float(request.form.get('amount') or 0.0)[cite: 13]
    notes = request.form.get('notes', 'Ulam / Register Drawer Starting Cash').strip()[cite: 13]
    staff_user = session.get('cashier_user') or session.get('admin_user') or 'Cashier'[cite: 13]

    if amount <= 0:
        flash("Change fund amount must be greater than zero.", "error")[cite: 13]
        return redirect(url_for('cashier_terminal'))[cite: 13]

    db.session.add(ChangeFund(fund_title=title, amount=amount, notes=notes, created_by=staff_user))[cite: 13]
    db.session.commit()[cite: 13]
...
def cashier_create_collection():
    cust_type = request.form.get('customer_type', 'REGISTERED')[cite: 13]
    item_choice_type = request.form.get('item_choice_type', 'PRODUCT')[cite: 13]
    product_id = request.form.get('product_id')[cite: 13]
    qty = int(request.form.get('quantity') or 1)[cite: 13]
    dining_opt = request.form.get('dining_option', 'TAKEOUT').upper()[cite: 13]
    notes = request.form.get('notes', '').strip()[cite: 13]
    
    prod_name = 'Custom Receivable Item'[cite: 13]
    unit_p = float(request.form.get('custom_amount') or 0.0)[cite: 13]

    if item_choice_type == 'PRODUCT' and product_id:
        prod = Product.query.get(int(product_id))[cite: 13]
        if prod:
            prod_name = prod.name[cite: 13]
            unit_p = prod.price[cite: 13]
    else:
        prod_name = request.form.get('custom_title', '').strip() or 'Custom Receivable Service'[cite: 13]

    amount = unit_p * qty[cite: 13]
    ...
    cust_id = None[cite: 13]
    cust_name = 'Walk-in Customer'[cite: 13]
    contact = 'N/A'[cite: 13]
    ...
    if cust_type == 'REGISTERED':
        ...
        if reg_id:
            ...
            if cust:
                ...
                cust_name = cust.name[cite: 13]
                contact = cust.contact[cite: 13]
                cust.outstanding_ar = (cust.outstanding_ar or 0.0) + amount[cite: 13]
    else:
        cust_name = request.form.get('custom_customer_name', '').strip() or 'Custom Account'[cite: 13]
        contact = request.form.get('custom_contact', '').strip() or 'N/A'[cite: 13]
    ...
    db.session.commit()[cite: 13]
    flash(f"For Collection ({dining_opt}) recorded for {cust_name}: {prod_name} x{qty} (₱{amount:,.2f})", "info")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]

@app.route('/pos/settle-collection/<int:order_id>', methods=['POST'])
@require_cashier
def cashier_settle_collection(order_id):
    order = Order.query.get_or_404(order_id)[cite: 13]
    pay_method = request.form.get('payment_method', 'CASH')[cite: 13]
    
    order.is_unpaid = False[cite: 13]
    order.status = 'COMPLETED'[cite: 13]
    order.payment_method = pay_method[cite: 13]
    order.payment_verified = True[cite: 13]

    is_same_day = (order.created_at.date() == datetime.utcnow().date())[cite: 13]
    earned = 0[cite: 13]

    if order.customer_id:
        cust = Customer.query.get(order.customer_id)[cite: 13]
        if cust:
            cust.outstanding_ar = max(0.0, (cust.outstanding_ar or 0.0) - order.total_amount)[cite: 13]
            cust.accumulated_spend = (cust.accumulated_spend or 0.0) + order.total_amount[cite: 13]
            
            if is_same_day:
                earned = int(order.total_amount // 30)[cite: 13]
                if earned > 0:
                    cust.points_balance = (cust.points_balance or 0.0) + earned[cite: 13]
                    db.session.add(RewardLedger(customer_id=cust.id, points_change=earned, reason=f"Same-Day Settled Credit #{order.id}"))[cite: 13]

    db.session.commit()[cite: 13]
    bonus_msg = f" (+{earned} pts earned for Same-Day payment!)" if earned > 0 else " (No points: paid after order date)"[cite: 13]
    flash(f"Collection #{order.id} for {order.customer_name} settled via {pay_method}!{bonus_msg}", "success")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]

@app.route('/pos/settle-customer-credit/<int:cust_id>', methods=['POST'])
@require_cashier
def cashier_settle_customer_credit(cust_id):
    cust = Customer.query.get_or_404(cust_id)[cite: 13]
    amount = float(request.form.get('amount') or cust.outstanding_ar)[cite: 13]
    pay_method = request.form.get('payment_method', 'CASH').upper()[cite: 13]
    ...
    amount_to_pay = min(amount, cust.outstanding_ar)[cite: 13]
    cust.outstanding_ar = max(0.0, cust.outstanding_ar - amount_to_pay)[cite: 13]
    cust.accumulated_spend = (cust.accumulated_spend or 0.0) + amount_to_pay[cite: 13]

    db.session.commit()[cite: 13]
    flash(f"Settled ₱{amount_to_pay:,.2f} credit balance for {cust.name} via {pay_method}!", "success")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]

@app.route('/pos/record-expense', methods=['POST'])
@require_cashier
def cashier_record_expense():
    title = request.form.get('title', '').strip()[cite: 13]
    amount = float(request.form.get('amount') or 0.0)[cite: 13]
    category = request.form.get('category', 'Supplies')[cite: 13]
    staff_user = session.get('cashier_user') or session.get('admin_user') or 'Cashier'[cite: 13]

    if not title or amount <= 0:
        flash("Please provide a valid title and amount.", "error")[cite: 13]
        return redirect(url_for('cashier_terminal'))[cite: 13]

    db.session.add(Expense(title=title, amount=amount, category=category, created_by=staff_user))[cite: 13]
    db.session.commit()[cite: 13]
    flash(f"Expense recorded: {title} (₱{amount:,.2f})", "success")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]

@app.route('/pos/record-vault-drop', methods=['POST'])
@require_cashier
def cashier_record_vault_drop():
    drop_num = int(request.form.get('drop_number') or 1)[cite: 13]
    amount = float(request.form.get('amount') or 0.0)[cite: 13]
    notes = request.form.get('notes', '').strip()[cite: 13]
    breakdown = request.form.get('cash_breakdown', '').strip()[cite: 13]
    staff_user = session.get('cashier_user') or session.get('admin_user') or 'Cashier'[cite: 13]

    if amount <= 0:
        flash("Drop amount must be greater than zero.", "error")[cite: 13]
        return redirect(url_for('cashier_terminal'))[cite: 13]

    db.session.add(VaultDrop(drop_number=drop_num, amount=amount, notes=notes, cash_breakdown=breakdown, created_by=staff_user))[cite: 13]
    db.session.commit()[cite: 13]
    flash(f"Vault Cash Drop #{drop_num} recorded: ₱{amount:,.2f}", "success")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]
...
def cashier_update_wifi_minutes(cust_id):
    cust = Customer.query.get_or_404(cust_id)[cite: 13]
    mins = request.form.get('wifi_minutes')[cite: 13]
    if mins is not None:
        try:
            cust.wifi_minutes_left = max(0, int(mins))[cite: 13]
            db.session.commit()[cite: 13]
            flash(f"Updated Wi-Fi time for {cust.name} to {cust.wifi_minutes_left} mins.", "success")[cite: 13]
        except ValueError:
            flash("Invalid minutes entered.", "error")[cite: 13]
    return redirect(url_for('cashier_terminal'))[cite: 13]
...
def verify_order(order_id):
    order = Order.query.get_or_404(order_id)[cite: 13]
    action = request.form.get('action')[cite: 13]
    ...
    if action == 'ACCEPT':
        ...
        if order.customer_id and order.payment_method != "CREDIT":
            ...
            if cust:
                ...
                if earned > 0:
                    cust.points_balance = (cust.points_balance or 0.0) + earned[cite: 13]
                    db.session.add(RewardLedger(customer_id=cust.id, points_change=earned, reason=f"Purchase Order #{order.id}"))[cite: 13]

        db.session.commit()[cite: 13]
        flash(f"Order #{order.id} accepted and completed. Details ready to print!", "success")[cite: 13]
    else:
        for item in order.items:
            if item.product_id:
                prod = Product.query.get(item.product_id)[cite: 13]
                if prod:
                    prod.stock += item.quantity[cite: 13]
        order.status = "CANCELLED"[cite: 13]
        db.session.commit()[cite: 13]
        flash(f"Order #{order.id} cancelled.", "info")[cite: 13]
...
def admin_dashboard():
    ensure_default_promos()[cite: 13]
    sort_by = request.args.get('sort', 'likes_desc')[cite: 13]

    query = Product.query[cite: 13]
    if sort_by == 'likes_desc': query = query.order_by(Product.total_likes.desc())[cite: 13]
    elif sort_by == 'price_asc': query = query.order_by(Product.price.asc())[cite: 13]
    elif sort_by == 'price_desc': query = query.order_by(Product.price.desc())[cite: 13]
    elif sort_by == 'name_asc': query = query.order_by(Product.name.asc())[cite: 13]
    elif sort_by == 'category': query = query.order_by(Product.category_name.asc(), Product.name.asc())[cite: 13]
    elif sort_by == 'featured': query = query.order_by(Product.is_featured.desc(), Product.name.asc())[cite: 13]
    elif sort_by == 'top_seller': query = query.order_by(Product.is_top_seller.desc(), Product.name.asc())[cite: 13]

    products = query.all()[cite: 13]
    categories = Category.query.all()[cite: 13]
    staff_members = Staff.query.all()[cite: 13]
    customers = Customer.query.order_by(Customer.id.desc()).all()[cite: 13]
    delivery_zones = DeliveryZone.query.all()[cite: 13]
    all_orders = Order.query.order_by(Order.created_at.desc()).limit(150).all()[cite: 13]
    all_expenses = Expense.query.order_by(Expense.created_at.desc()).all()[cite: 13]
    vault_drops = VaultDrop.query.order_by(VaultDrop.created_at.desc()).all()[cite: 13]
    promotions = PromotionTracker.query.order_by(PromotionTracker.created_at.desc()).all()[cite: 13]
    ...
    week_ago = datetime.utcnow() - timedelta(days=7)[cite: 13]
    month_ago = datetime.utcnow() - timedelta(days=30)[cite: 13]

    daily_orders = Order.query.filter(db.func.date(Order.created_at) == today, Order.status == 'COMPLETED').all()[cite: 13]
    weekly_orders = Order.query.filter(Order.created_at >= week_ago, Order.status == 'COMPLETED').all()[cite: 13]
    monthly_orders = Order.query.filter(Order.created_at >= month_ago, Order.status == 'COMPLETED').all()[cite: 13]
    all_completed = Order.query.filter_by(status='COMPLETED').all()[cite: 13]

    daily_exp = db.session.query(db.func.coalesce(db.func.sum(Expense.amount), 0.0)).filter(db.func.date(Expense.created_at) == today).scalar() or 0.0[cite: 13]
    weekly_exp = db.session.query(db.func.coalesce(db.func.sum(Expense.amount), 0.0)).filter(Expense.created_at >= week_ago).scalar() or 0.0[cite: 13]
    monthly_exp = db.session.query(db.func.coalesce(db.func.sum(Expense.amount), 0.0)).filter(Expense.created_at >= month_ago).scalar() or 0.0[cite: 13]
    total_exp_all = sum(e.amount for e in all_expenses)[cite: 13]

    daily_vault = db.session.query(db.func.coalesce(db.func.sum(VaultDrop.amount), 0.0)).filter(db.func.date(VaultDrop.created_at) == today).scalar() or 0.0[cite: 13]
    weekly_vault = db.session.query(db.func.coalesce(db.func.sum(VaultDrop.amount), 0.0)).filter(VaultDrop.created_at >= week_ago).scalar() or 0.0[cite: 13]
    monthly_vault = db.session.query(db.func.coalesce(db.func.sum(VaultDrop.amount), 0.0)).filter(VaultDrop.created_at >= month_ago).scalar() or 0.0[cite: 13]
    all_vault = db.session.query(db.func.coalesce(db.func.sum(VaultDrop.amount), 0.0)).scalar() or 0.0[cite: 13]
    ...
    fin_daily = calc_period(daily_orders, daily_exp, daily_vault)[cite: 13]
    fin_weekly = calc_period(weekly_orders, weekly_exp, weekly_vault)[cite: 13]
    fin_monthly = calc_period(monthly_orders, monthly_exp, monthly_vault)[cite: 13]
    fin_all = calc_period(all_completed, total_exp_all, all_vault)[cite: 13]

    product_sales_stats = {}[cite: 13]
    food_revenue_total = 0.0[cite: 13]
    service_revenue_total = 0.0[cite: 13]
    ...
    for o in all_completed:
        for it in o.items:
            ...
            if '[Service]' in pname or o.order_type == 'SERVICE/MISC' or 'Printing' in pname:
                ...
                food_revenue_total += (it.subtotal or 0.0)[cite: 13]

    food_revenue_total += all_vault[cite: 13]
    total_ar = sum((c.outstanding_ar or 0.0) for c in customers)[cite: 13]
    ...
    return render_template('admin.html', 
                           ...
                           fin_all=fin_all, 
                           total_ar=total_ar, 
                           current_sort=sort_by)[cite: 13]

@app.route('/admin/allocate-vault-drop/<int:drop_id>', methods=['POST'])
@require_admin
def admin_allocate_vault_drop(drop_id):
    drop = VaultDrop.query.get_or_404(drop_id)[cite: 13]
    cust_id = request.form.get('customer_id')[cite: 13]
    product_id = request.form.get('product_id')[cite: 13]
    allocated_amount = float(request.form.get('amount') or 0.0)[cite: 13]
    qty = int(request.form.get('quantity') or 1)[cite: 13]

    if allocated_amount <= 0 or allocated_amount > drop.amount:
        flash("Allocated amount must be between ₱0.01 and the remaining drop balance.", "error")[cite: 13]
        return redirect(url_for('admin_dashboard'))[cite: 13]

    cust = Customer.query.get(int(cust_id)) if cust_id else None[cite: 13]
    prod = Product.query.get(int(product_id)) if product_id else None[cite: 13]

    drop.amount = max(0.0, drop.amount - allocated_amount)[cite: 13]
    ...
    if cust:
        cust.accumulated_spend = (cust.accumulated_spend or 0.0) + allocated_amount[cite: 13]
        earned = int(allocated_amount // 30)[cite: 13]
        if earned > 0:
            cust.points_balance = (cust.points_balance or 0.0) + earned[cite: 13]
            db.session.add(RewardLedger(
                customer_id=cust.id,
    ...
    db.session.commit()[cite: 13]
    flash(f"Allocated ₱{allocated_amount:,.2f} from Drop #{drop.drop_number} into a dedicated Order #{order.id}!", "success")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]
...
def admin_reassign_order(order_id):
    order = Order.query.get_or_404(order_id)[cite: 13]
    new_cust_id = request.form.get('customer_id')[cite: 13]

    if not new_cust_id:
        flash("Please select a target customer.", "error")[cite: 13]
        return redirect(url_for('admin_dashboard'))[cite: 13]

    new_cust = Customer.query.get_or_404(int(new_cust_id))[cite: 13]
    old_cust = Customer.query.get(order.customer_id) if order.customer_id else None[cite: 13]

    if old_cust and order.status == 'COMPLETED':
        earned = int(order.total_amount // 30)[cite: 13]
        old_cust.points_balance = max(0.0, (old_cust.points_balance or 0.0) - earned)[cite: 13]
        old_cust.accumulated_spend = max(0.0, (old_cust.accumulated_spend or 0.0) - order.total_amount)[cite: 13]

    order.customer_id = new_cust.id[cite: 13]
    order.customer_name = new_cust.name[cite: 13]
    order.contact_number = new_cust.contact[cite: 13]

    if order.status == 'COMPLETED':
        earned_new = int(order.total_amount // 30)[cite: 13]
        new_cust.accumulated_spend = (new_cust.accumulated_spend or 0.0) + order.total_amount[cite: 13]
        if earned_new > 0:
            new_cust.points_balance = (new_cust.points_balance or 0.0) + earned_new[cite: 13]
            db.session.add(RewardLedger(
    ...
    db.session.commit()[cite: 13]
    flash(f"Transaction #{order.id} reassigned to '{new_cust.name}'.", "success")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]

@app.route('/admin/update-expense/<int:expense_id>', methods=['POST'])
@require_admin
def admin_update_expense(expense_id):
    exp = Expense.query.get_or_404(expense_id)[cite: 13]
    exp.title = request.form.get('title', exp.title).strip()[cite: 13]
    exp.amount = float(request.form.get('amount') or exp.amount)[cite: 13]
    exp.category = request.form.get('category', exp.category).strip()[cite: 13]
    db.session.commit()[cite: 13]
    flash(f"Expense #{exp.id} updated successfully.", "success")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]
...
def admin_delete_expense(expense_id):
    exp = Expense.query.get_or_404(expense_id)[cite: 13]
    db.session.delete(exp)[cite: 13]
    db.session.commit()[cite: 13]
    flash(f"Expense record removed.", "info")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]
...
def admin_delete_product(product_id):
    ...
    OrderItem.query.filter_by(product_id=prod.id).update({'product_id': None})[cite: 13]
    ProductLike.query.filter_by(product_id=prod.id).delete()[cite: 13]
    ProductComment.query.filter_by(product_id=prod.id).delete()[cite: 13]

    db.session.delete(prod)[cite: 13]
    db.session.commit()[cite: 13]
    flash(f"Product '{prod_name}' permanently deleted.", "info")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]
...
def admin_revert_order(order_id):
    ...
    for item in order.items:
        if item.product_id:
            p = Product.query.get(item.product_id)[cite: 13]
            if p:
                p.stock += item.quantity[cite: 13]

    if order.customer_id and order.status == 'COMPLETED':
        cust = Customer.query.get(order.customer_id)[cite: 13]
        if cust:
            earned = int(order.total_amount // 30)[cite: 13]
            cust.points_balance = max(0.0, (cust.points_balance or 0.0) - earned)[cite: 13]
            cust.accumulated_spend = max(0.0, (cust.accumulated_spend or 0.0) - order.total_amount)[cite: 13]
            db.session.add(RewardLedger(
                customer_id=cust.id,
                points_change=-earned,
    ...
    db.session.delete(order)[cite: 13]
    db.session.commit()[cite: 13]
...
def admin_purge_dummy_orders(cust_id):
    cust = Customer.query.get_or_404(cust_id)[cite: 13]
    dummy_orders = Order.query.filter_by(customer_id=cust.id).all()[cite: 13]
    for o in dummy_orders:
        db.session.delete(o)[cite: 13]
    cust.points_balance = 0.0[cite: 13]
    cust.accumulated_spend = 0.0[cite: 13]
    cust.outstanding_ar = 0.0[cite: 13]
    RewardLedger.query.filter_by(customer_id=cust.id).delete()[cite: 13]
    db.session.commit()[cite: 13]
...
def admin_delete_customer(cust_id):
    cust = Customer.query.get_or_404(cust_id)[cite: 13]
    Order.query.filter_by(customer_id=cust.id).delete()[cite: 13]
    RewardLedger.query.filter_by(customer_id=cust.id).delete()[cite: 13]
    ProductLike.query.filter_by(customer_id=cust.id).delete()[cite: 13]
...
def admin_update_logo():
    new_logo = request.form.get('logo_url', '').strip()[cite: 13]
    if new_logo:
        setting = StoreSetting.query.filter_by(key='logo_url').first()[cite: 13]
        if not setting:
            db.session.add(StoreSetting(key='logo_url', value=new_logo))[cite: 13]
        else:
            setting.value = new_logo[cite: 13]
        db.session.commit()[cite: 13]
        flash("Company logo updated across all portals.", "success")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]

@app.route('/admin/add-product', methods=['POST'])
@require_admin
def admin_add_product():
    name = request.form.get('name', '').strip()[cite: 13]
    category_name = request.form.get('category_name', 'Meals')[cite: 13]
    price = float(request.form.get('price') or 0.0)[cite: 13]
    stock = int(request.form.get('stock') or 100)[cite: 13]
    image_url = request.form.get('image_url', '').strip()[cite: 13]
    start_t = request.form.get('available_start_time', '').strip() or None[cite: 13]
    end_t = request.form.get('available_end_time', '').strip() or None[cite: 13]
    is_featured = bool(request.form.get('is_featured'))[cite: 13]
    is_top_seller = bool(request.form.get('is_top_seller'))[cite: 13]
...
def admin_batch_update_products():
    for pid in request.form.getlist('product_id'):[cite: 13]
        prod = Product.query.get(pid)[cite: 13]
        if prod:
            prod.price = float(request.form.get(f'price_{pid}') or prod.price)[cite: 13]
            prod.stock = int(request.form.get(f'stock_{pid}') or prod.stock)[cite: 13]
            prod.image_url = request.form.get(f'image_url_{pid}', '').strip()[cite: 13]
            prod.available_start_time = request.form.get(f'available_start_time_{pid}', '').strip() or None[cite: 13]
            prod.available_end_time = request.form.get(f'available_end_time_{pid}', '').strip() or None[cite: 13]
            prod.is_active = (f'is_active_{pid}' in request.form)[cite: 13]
            prod.is_featured = (f'is_featured_{pid}' in request.form)[cite: 13]
            prod.is_top_seller = (f'is_top_seller_{pid}' in request.form)[cite: 13]
    db.session.commit()[cite: 13]
    flash("Bulk product catalog updated.", "success")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]
...
def admin_toggle_credit(cust_id):
    cust = Customer.query.get_or_404(cust_id)[cite: 13]
    cust.is_credit_eligible = not cust.is_credit_eligible[cite: 13]
    limit = request.form.get('credit_limit')[cite: 13]
    if limit:
        cust.credit_limit = float(limit)[cite: 13]
    db.session.commit()[cite: 13]
    flash(f"Credit eligibility updated for {cust.name}.", "success")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]
...
def admin_manage_delivery_zones():
    action = request.form.get('action')[cite: 13]
    if action == 'ADD':
        place = request.form.get('place_name')[cite: 13]
        brgy = request.form.get('barangay')[cite: 13]
        rate = float(request.form.get('rate') or 40.0)[cite: 13]
        dist = request.form.get('distance')[cite: 13]
        note = request.form.get('note')[cite: 13]
        db.session.add(DeliveryZone(place_name=place, barangay=brgy, rate=rate, distance=dist, note=note))[cite: 13]
    elif action == 'DELETE':
        zid = request.form.get('zone_id')[cite: 13]
        zone = DeliveryZone.query.get(zid)[cite: 13]
        if zone:
            db.session.delete(zone)[cite: 13]
    db.session.commit()[cite: 13]
    flash("Delivery zones updated.", "success")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]
...
def admin_reset_customer_pin(cust_id):
    cust = Customer.query.get_or_404(cust_id)[cite: 13]
    new_pin = request.form.get('new_pin', '').strip()[cite: 13]
    if not new_pin or len(new_pin) < 4:
        flash("PIN must be at least 4 digits.", "error")[cite: 13]
    else:
        cust.pin_hash = generate_password_hash(new_pin)[cite: 13]
        db.session.commit()[cite: 13]
        flash(f"PIN for customer '{cust.name}' reset to {new_pin}.", "success")[cite: 13]
    return redirect(url_for('admin_dashboard'))[cite: 13]

@app.route('/admin/toggle-promo/<int:promo_id>', methods=['POST'])
@require_admin
def admin_toggle_promo(promo_id):
    promo = PromotionTracker.query.get_or_404(promo_id)[cite: 13]
    promo.is_active = not promo.is_active[cite: 13]
    if promo.is_active:
        promo.created_at = datetime.utcnow()[cite: 13]
    db.session.commit()[cite: 13]
...
def admin_toggle_promo_visibility(promo_id):
    promo = PromotionTracker.query.get_or_404(promo_id)[cite: 13]
    promo.is_visible = not getattr(promo, 'is_visible', True)[cite: 13]
    db.session.commit()[cite: 13]
...
def customer_login():
    if request.method == 'POST':
        contact = request.form.get('contact', '').strip()[cite: 13]
        pin = request.form.get('pin', '').strip()[cite: 13]
        cust = Customer.query.filter_by(contact=contact).first()[cite: 13]
        if cust and check_password_hash(cust.pin_hash, pin):
            session['customer_id'] = cust.id[cite: 13]
            today = date.today()
            ...
            cust.last_active_at = datetime.utcnow()
            db.session.commit()
            return redirect(url_for('customer_dashboard'))[cite: 13]
        flash("Invalid Contact or PIN.", "error")[cite: 13]
    return render_template('customer_login.html')[cite: 13]
...
def customer_register():
    ...
    if request.method == 'POST':
        name = request.form.get('name', '').strip()[cite: 13]
        email = request.form.get('email', '').strip()[cite: 13]
        contact = request.form.get('contact', '').strip()[cite: 13]
        messenger = request.form.get('fb_messenger', '').strip()[cite: 13]
        pin = request.form.get('pin', '').strip()[cite: 13]
        address = request.form.get('default_address', '').strip()[cite: 13]
        landmark = request.form.get('default_landmark', '').strip()[cite: 13]
        ref = request.form.get('referral_code', '').strip() or ref_code

        if Customer.query.filter_by(contact=contact).first():
            flash("Contact number already registered.", "error")[cite: 13]
            return redirect(url_for('customer_register'))[cite: 13]
        ...
        db.session.add(new_cust)[cite: 13]
        db.session.flush()[cite: 13]
        ...
        db.session.commit()[cite: 13]
        session['customer_id'] = new_cust.id[cite: 13]
...
def customer_dashboard():
    ...
    if not cust:
        session.pop('customer_id', None)[cite: 13]
    ...
    my_orders = Order.query.filter_by(customer_id=cust.id).order_by(Order.created_at.desc()).all()[cite: 13]
    promo_burger = PromotionTracker.query.filter_by(promo_code='BURGER_FRIES_50', is_visible=True).first()[cite: 13]
    promo_nachos = PromotionTracker.query.filter_by(promo_code='BEEFY_NACHOS_75', is_visible=True).first()[cite: 13]
...
def customer_logout():
    if 'customer_id' in session:
        try:
            cust = Customer.query.get(session['customer_id'])[cite: 13]
            if cust and hasattr(cust, 'last_active_at'):
                cust.last_active_at = datetime.utcnow() - timedelta(hours=1)[cite: 13]
                db.session.commit()[cite: 13]
        except Exception:
            db.session.rollback()[cite: 13]
    session.pop('customer_id', None)[cite: 13]
    flash("Successfully logged out.", "info")[cite: 13]
    return redirect(url_for('store_catalog'))[cite: 13]
...
def customer_reserve_promo_by_code(promo_code):
    ...
    ensure_default_promos()[cite: 13]
    cust = Customer.query.get_or_404(session['customer_id'])[cite: 13]
    promo = PromotionTracker.query.filter_by(promo_code=promo_code).first()[cite: 13]

    if not promo or not promo.is_active:
        flash("This promotion campaign is currently archived.", "info")[cite: 13]
        return redirect(url_for('customer_dashboard'))[cite: 13]

    days_active = (datetime.utcnow() - promo.created_at).days[cite: 13]
    if days_active > 3:
        promo.is_active = False[cite: 13]
        db.session.commit()[cite: 13]
        flash("This 3-day promotion campaign has ended and is now archived.", "info")[cite: 13]
        return redirect(url_for('customer_dashboard'))[cite: 13]
...
def update_profile_pic():
    if 'customer_id' not in session:
        return redirect(url_for('customer_login'))[cite: 13]
    cust = Customer.query.get(session['customer_id'])[cite: 13]
    img_url = request.form.get('profile_image', '').strip()[cite: 13]
    if img_url:
        cust.profile_image = img_url[cite: 13]
        db.session.commit()[cite: 13]
        flash("Profile picture updated!", "success")[cite: 13]
    return redirect(url_for('customer_dashboard'))[cite: 13]
...
def staff_login():
    target = request.args.get('target', '').lower()[cite: 13]
    if request.method == 'POST':
        user = request.form.get('username', '').strip().lower()[cite: 13]
        pin = request.form.get('pin', '').strip()[cite: 13]
        staff = Staff.query.filter(db.func.lower(Staff.username) == user).first()[cite: 13]

        if staff and check_password_hash(staff.pin_hash, pin):
            if staff.role == 'ADMIN':
                session['admin_id'] = staff.id[cite: 13]
                session['admin_user'] = staff.username[cite: 13]
                return redirect(url_for('admin_dashboard'))[cite: 13]
            elif staff.role == 'CASHIER':
                session['cashier_id'] = staff.id[cite: 13]
                session['cashier_user'] = staff.username[cite: 13]
                return redirect(url_for('cashier_terminal'))[cite: 13]
        flash('Invalid Username or PIN.', 'error')[cite: 13]
    return render_template('staff_login.html', target=target)[cite: 13]

@app.route('/staff/logout')
def staff_logout():
    role = request.args.get('role')[cite: 13]
    if role == 'admin': session.pop('admin_id', None); session.pop('admin_user', None)[cite: 13]
    elif role == 'cashier': session.pop('cashier_id', None); session.pop('cashier_user', None)[cite: 13]
    else: session.clear()[cite: 13]
