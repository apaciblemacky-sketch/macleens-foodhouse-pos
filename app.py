...
with app.app_context():
    try:
        ...
        promo_cols = [c['name'] for c in inspector.get_columns('promotion_tracker')]
        ...
        with db.engine.begin() as conn:
            ...
            if 'dining_option' not in order_cols:
                conn.execute(text("ALTER TABLE \"order\" ADD COLUMN dining_option VARCHAR(20) DEFAULT 'DINE-IN';"))
            if 'is_visible' not in promo_cols:
                conn.execute(text("ALTER TABLE promotion_tracker ADD COLUMN is_visible BOOLEAN DEFAULT TRUE;"))
