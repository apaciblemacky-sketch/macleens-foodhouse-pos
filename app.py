...
with app.app_context():
    try:
        ...
        for user, pin, role in default_roles:
            st = Staff.query.filter_by(username=user).first()[cite: 13]
            if not st:
                db.session.add(Staff(username=user, pin_hash=generate_password_hash(pin), role=role))[cite: 13]
            else:
                st.pin_hash = generate_password_hash(pin)[cite: 13]
                st.role = role[cite: 13]
        db.session.commit()[cite: 13]
